...
def get_film_ids(amount = 501, year_start = 2019, year_end = 2023):
    film_ids = []
    print("getting film id...")
    for page in tqdm(range(1,amount)):
...
def film_data(film_ids):  
    ...
    for id in tqdm(film_ids[0]):
        ...
        with client.write(f'/user/hadoop/actor_pair.csv', encoding = 'utf-8') as writer:
            temp.to_csv(writer)
        films = pd.concat([films, pd.Series(film).to_frame().T], ignore_index=True)
        ...
        temp['weight'] = temp['movie'].apply(lambda x: films[films['id'] == x]['popularity'])
        weight_temp = temp[['actor1','actor2','weight']].groupby(['actor1','actor2']).mean().reset_index()
        with client.write(f"/user/hadoop/weight/weight_{id}.csv",  encoding = 'utf-8', append = True) as writer:
            weight_temp.to_csv(writer)
...
if __name__ == '__main__':
    ...
    while year_start > 2015:
        film_ids = []
        film_ids.append(get_film_ids(500, year_start,2023))
